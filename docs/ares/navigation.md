...
* The List: Displays a summary of recent campaigns, including their names and completion times.

* Campaign Details: Selecting a campaign opens a detailed view with timestamps, experiment count, tags, notes, and a hierarchical tree of experiments, steps, and commands, including analyzer and parameter information.

## 4. Data Viewer
_Access: Sidebar -> "Data Viewer"_

This is the deep-dive analysis tool. While Execution History summarizes individual runs, Data Viewer (the Data Explorer page in the UI) focuses on tabular datasets for completed campaigns and device state.

* **Device State tab:** Use filters (time window, minimum sample interval, campaign and experiment selection) to generate state tables for one or more devices. Choose display modes (separate per device, wide, or long) and export the current view as CSV or multiple tables as a ZIP.

* **Campaign Data tab:** Select a completed campaign to generate experiment tables. Tags and notes recorded with the campaign are shown alongside the tables, and you can export a single table or all tables for that campaign.

Execution History is ideal for inspecting the structure of a run (experiments, steps, commands), while Data Viewer is designed for aggregating and exporting the underlying data for analysis.
